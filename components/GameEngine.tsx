...
const MAX_HUMANITY = QUESTIONS.reduce((total, question) => {
  const scores = question.scores ?? [0];
...
const calculateScore = (answers: Record<number, number>) => {
  return Object.entries(answers).reduce((total, [key, choiceIndex]) => {
    const questionIndex = Number(key);
    const score = QUESTIONS[questionIndex]?.scores?.[choiceIndex] ?? 0;
    return total + score;
  }, 0);
};
...
export default function GameEngine() {
  ...
  const [phase, setPhase] = useState<"question" | "fade-out" | "bar" | "fade-in">(
    "question"
  );
  ...
  const startResolution = useCallback(
    (questionIndex: number, choiceIndex: number) => {
      ...
      setTimeout(() => {
        ...
        setPhase("bar");
        const score = QUESTIONS[questionIndex]?.scores?.[choiceIndex] ?? 0;
        setHumanity((prev) => Math.min(MAX_HUMANITY, prev + score));
        setPulseIndex(null);
      }, FADE_DURATION_MS);
      setTimeout(() => {
        ...
        if (questionIndex < QUESTIONS.length - 1) {
          ...
          const finalScore = Math.round((MAX_HUMANITY * FINAL_PERCENT) / 100);
          setHumanity(finalScore);
          setCompleted(true);
  ...
  const resolveQuestion = useCallback(
    (questionIndex: number) => {
      ...
      startResolution(questionIndex, choiceIndex);
    },
        ...
        <div className="relative overflow-hidden rounded-[32px] border border-neon-cyan/30 bg-midnight/70 px-5 py-6 shadow-[0_0_40px_rgba(45,250,255,0.25)] sm:px-10 sm:py-10">
          ...
          <div className="relative mx-auto flex max-w-2xl flex-col items-center gap-4 rounded-3xl border border-neon-cyan/30 bg-slate-950/70 px-6 py-8 text-center shadow-[0_0_24px_rgba(45,250,255,0.2)] sm:px-10 sm:py-10">
            ...
            <h2 className="tron-font text-[clamp(1.4rem,3.2vw,3.2rem)] font-semibold text-neon-cyan">
              Humanité : {Math.round((humanity / MAX_HUMANITY) * 100)}%
            </h2>
  ...
  return (
    <div
      className={`flex h-full min-h-0 flex-col items-center gap-4 text-center ${
        showQA ? "justify-between" : "justify-center"
      }`}
    >
      {showQA ? (
        <div
          ...
          return (
            <NeonButton
              ...
              onClick={() => lockAnswer(index, activeIndex)}
                  disabled={phase !== "question" || (isLocked && !isSelected)}
              className={`min-h-[clamp(50px,7.5vh,80px)] items-center justify-between gap-6 rounded-3xl border-2 px-6 py-5 text-left tracking-normal transition duration-300 hover:-translate-y-0.5 hover:bg-neon-cyan/10 hover:border-neon-cyan hover:shadow-neon ${
                ...
                shouldPulse
                  ? "animate-[pulseGlow_0.35s_ease-in-out_1] shadow-[0_0_24px_rgba(183,255,42,0.85)]"
                  : ""
              } normal-case`}
            >
          <span
      ...
      <div className="w-full max-w-5xl">
        <HumanityBar
          ...
          max={MAX_HUMANITY}
          isLedMode={isLedMode}
          transitionMs={phase === "bar" ? BAR_ANIM_MS : 700}
        />
      </div>
    </div>
  );
